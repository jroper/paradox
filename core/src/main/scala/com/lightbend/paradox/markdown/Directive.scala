...
case class RefDirective(ctx: Writer.Context)
  ...
  def render(node: DirectiveNode, visitor: Visitor, printer: Printer): Unit = {
    ...
    try {
      val path = ctx.pageMappings(source)
      new ExpLinkNode("", check(node, path), node.contentsNode).accept(visitor)
    } catch {
  ...
  private def check(node: DirectiveNode, path: String): String = {
    ctx.paths.get(Path.resolve(page.path, path)) match {
      case Some(target) =>
        if (path.contains("#")) {
        ...
        path
      case None =>
        ctx.error(s"Unknown page [$path]", node)
        ""
    }
  }
}
...
case class SnipDirective(ctx: Writer.Context)
  ...
  def render(node: DirectiveNode, visitor: Visitor, printer: Printer): Unit = {
    try {
      ...
      case e: FileNotFoundException => ctx.error("Could not find file for snippet", node)
      case e: SnippetException      => ctx.error(e.getMessage, node)
    }
  }
