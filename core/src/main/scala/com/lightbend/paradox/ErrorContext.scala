...
/**
 * Ideally we would return errors as part of the result, but the only way to return an error in pegdown is to throw,
 * and that stops processing, which means we can only throw one error per file, which, if the file has lots of
 * problems, means we can only fix one at a time. So, instead we collect errors in a thread local, to display at the
 * end.
 */
trait ErrorContext {
...
class ErrorCollector extends ErrorContext {
  ...
  def logErrors(log: String => Unit) = {
    val totalErrors = errors.toList.distinct
    // First log general errors
    totalErrors.foreach {
      case ParadoxError(msg, None, _) => log(msg)
      case _                          =>
    }
      ...
      case (page, errors) =>
        ...
        errors.sortBy(_._3.getOrElse(0)).foreach {
          case (error, _, Some(idx)) =>
            ...
            log(s"$error at ${page.getAbsolutePath}:$lineNo")
            line.foreach { l =>
              log(l)
              log(l.take(colNo - 1).map { case '\t' => '\t'; case _ => ' ' } + "^")
            }
          case (error, _, _) =>
            log(s"$error at ${page.getAbsolutePath}")
