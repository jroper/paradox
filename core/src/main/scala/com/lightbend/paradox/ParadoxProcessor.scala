...
class ParadoxProcessor(reader: Reader = new Reader, writer: Writer = new Writer) {
  ...
  def process(
    ...
    warn:               String => Unit,
    error:              String => Unit): Either[String, Seq[(File, String)]] = {
    ...
    def render(location: Option[Location[Page]], rendered: Seq[(File, String)] = Seq.empty): Seq[(File, String)] = location match {
      case Some(loc) =>
        ...
        val writerContext = Writer.Context(loc, pages, reader, writer, new PagedErrorContext(errorCollector, page), currentMapping, sourceSuffix, targetSuffix, groups, pageProperties)
    ...
    if (errorCollector.hasErrors) {
      errorCollector.logErrors(error)
  ...
  def validate(
    ...
    def validate(location: Option[Location[Page]]): Unit = location match {
      case Some(loc) =>
        val page = loc.tree.label
        println("Validating " + page.file)
        val pageProperties = properties ++ page.properties.get
        val currentMapping = Path.generateTargetFile(Path.relativeLocalPath(page.rootSrcPage, page.file.getPath), globalPageMappings)
        val writerContext = Writer.Context(loc, pages, reader, writer, new PagedErrorContext(errorCollector, page), currentMapping, "", "", groups, pageProperties)
    ...
    linkCapturer.allLinks(siteBaseUrl).foreach {
      ...
      case CapturedLinkWithSources(CapturedRelativeLink(path, fragment), sources) if !ignore(path) =>
        fullSite.get(path) match {
          case Some(file) =>
            fragment.foreach { f =>
              validateFragment(Source.fromFile(file, "UTF-8").mkString, f, sources, errorCollector)
            }
          case None =>
            sources.foreach {
    ...
    try {
      if (conn.getResponseCode != 200) {
        ...
        if (uri.getFragment != null) {
          ...
          validateFragment(content, uri.getFragment, sources, errorContext)
        }
      }
    } finally {
      conn.disconnect()
  ...
  private def validateFragment(content: String, fragment: String, sources: List[(File, Node)], errorContext: ErrorContext) = {
    // todo
  }
